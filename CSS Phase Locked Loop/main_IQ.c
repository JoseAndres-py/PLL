...
void main()
{
   ...
   // Pll routine (1ph):
   //---------------------------------------------------------------------------
   SPLL_1ph_IQ_init(GRID_FREQ,((float)(1.0/ISR_FREQUENCY)), &spll1);
...
interrupt void  adc_isr(void)
{
    ...
    //---------------------------------------------------------------------------
    spll1.AC_input = _IQ21((float)GridMeas/3000);
    ...
    SampleTable[i] = spll1.AC_input;
    PLLTable[i] = spll1.theta[0];

    Reg_32 GPIO_org; Reg_8 theta;
    theta.all = _IQsat(_IQ21mpy(spll1.sin[0], 56),255,0);
